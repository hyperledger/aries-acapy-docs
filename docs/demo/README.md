---
title: Aries Cloud Agent Python (ACA-Py) Demos
---
...
Note that when the controller starts up the agent, it prints out the ACA-Py startup command with _all_ parameters - you can inspect this command to see what parameters are provided in each case. For more details on the parameters, just start ACA-Py with the `--help` parameter, for example:
...
### Multi-tenancy _with Mediation_!!!
...
Run the demo (Alice or Faber) specifying _both_ options:

```bash
./run_demo faber --multitenant --mediation
```

This works exactly as the _vanilla_ multi-tenancy, except that all connections are mediated.
