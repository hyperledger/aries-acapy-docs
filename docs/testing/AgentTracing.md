...
You can use the `EFK` stack in the [EFK sub-directory](https://github.com/hyperledger/aries-cloudagent-python/tree/0.7.5/demo/EFK-stack) as a target for trace events, just start the EFK stack using the docker-compose file and then in two separate bash shells, startup the demo as follows:
